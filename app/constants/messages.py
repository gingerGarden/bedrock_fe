...
class AdminMsg:
    """
    관리자 전용 페이지 접근 시 권한이 없는 사용자가 호출했을 때의 메시지.
    """
    NO_ADMIN: Final[str] = "해당 페이지는 관리자 계정만 이용 가능합니다."
...
* **FloraGenesis** : 천연물 소재 분석 시스템
* **PANCDR** : 항암 약물 반응 예측 시스템
* **Dashboard** : 사용자 통계 및 주요 지표 시각화
* **Setting** : API Key 신청 등 계정별 설정 (개발자 전용)
